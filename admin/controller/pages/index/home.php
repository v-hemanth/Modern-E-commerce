...
class ControllerPagesIndexHome extends AController
{
    ...
    public function main()
    {
        ...
        if ($this->groupID == self::TOP_ADMIN_GROUP || $this->permissions['access']['catalog/review']) {
            ...
            $this->view->assign(
                'total_review_approval', $this->model_catalog_review->getTotalReviewsAwaitingApproval()
            );
        } else {
        ...
        if ($this->groupID == self::TOP_ADMIN_GROUP || $this->permissions['access']['sale/customer']) {
            ...
            $this->view->assign(
                'total_customer_approval', $this->model_sale_customer->getTotalCustomersAwaitingApproval()
            );
        ...
        if ($this->groupID == self::TOP_ADMIN_GROUP || $this->permissions['access']['sale/order']) {
            ...
            //sale overview
            $this->view->assign(
                'total_sale', $this->currency->format(
                $this->model_sale_order->getTotalSales(), $this->config->get('config_currency')
            )
            );
            $this->view->assign(
                'total_sale_year', $this->currency->format(
                $this->model_sale_order->getTotalSalesByYear(date('Y')),
